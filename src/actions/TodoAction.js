...
const TodoAction = {
    ...
    toggleAll: todos => dispatch => {
        ...
        const axArray = todos.map(todo => ax.put(`/${todo.id}`, {isDone: newToggleAll}));
    ...
    deleteCompleted: todos => dispatch => {
        const axArray = todos.filter(todo => todo.isDone).map( v => ax.delete(`/${v.id}`));
